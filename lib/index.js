...
server.route({
  ...
  handler: function (req, rep) {
    ...
    let sort = {};
    let message = {};
    // console.log(req.payload.item.message.message)
    // date yesterday/today/tomorrow
    // query.mealDate = '20161129';
    query.mealDate = moment().format('YYYYMMDD');
    ...
    col.find(query).sort(sort).toArray((err, docs) => {
      ...
      */
      ...
      if(docs.length > 0) {
        message = {
          color: 'green',
          message: `${docs[0].title}:${docs[0].component.carb}, ${docs[1].title}:${docs[1].component.carb}, ${docs[2].title}:${docs[2].component.carb}`,
          notify: false,
          message_format: 'text'
        };
      } else {
